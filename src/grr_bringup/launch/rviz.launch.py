...
def generate_launch_description():
    ...
    xacro_file = os.path.join(pkg_path,'urdf', 'robots','bloookdstone.urdf.xacro')
    grr_description_config = xacro.process_file(xacro_file)

    # Save the URDF file
    current_script_path = os.path.dirname(os.path.realpath(__file__))
    pkg_path = os.path.join(current_script_path, '..', '..', 'grr_description')
    urdf_save_path = os.path.join(pkg_path, "bloookdstone.urdf")
    ...
    rviz2_config_path = os.path.join(get_package_share_directory('grr_bringup'), 'config/view.rviz')
